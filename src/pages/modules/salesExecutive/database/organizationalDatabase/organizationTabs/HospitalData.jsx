...
const HospitalData = ({ formik, isReadOnly = false }) => {
  ...
  useEffect(() => {
    fetchSpeciality();
    fetchSurgeryType();
  }, [fetchSpeciality, fetchSurgeryType]);

  const specialities = formik.values?.hospitalData?.specialities || [];

  // Create speciality options from fetched data
  const specialityOptions = useMemo(() => {
    if (!speciality) return [];
    return speciality.map((spec) => ({
      value: spec._id || spec.name,
      label: spec.name,
    }));
  }, [speciality]);
    ...
    <div className="p-4">
      ...
      <div className="p-6 pt-0 bg-white rounded-md">
        ...
        {specialities.map((spec, specIndex) => {
          // Filter out already selected specialities
