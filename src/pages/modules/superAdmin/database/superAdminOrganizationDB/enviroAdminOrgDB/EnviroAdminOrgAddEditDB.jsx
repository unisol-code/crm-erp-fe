...
const EnviroAdminOrgAddEditDB = ({ mode = "add" }) => {
  ...
  const initialServices = (servicesOfferedDrop || []).reduce((acc, s) => {
    acc[s] = false;
    return acc;
  }, {});
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      console.log("Enviro Org data", values);
      if (isEditMode) {
  ...
  useEffect(() => {
    if (enviroAdminOrgDetails) {
      ...
      formik.setValues({
        ...
        servicesOffered: arrayToCheckboxObj(d.servicesOffered, initialServices),
  ...
  }, [enviroAdminOrgDetails, servicesOfferedDrop]);
